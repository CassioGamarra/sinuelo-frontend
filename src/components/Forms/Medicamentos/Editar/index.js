...
export default function FormEditarMedicamento(props) {
  ...
  async function buscarMedicamento() {
    handleOpen();
    try {
      const getDadosMedicamento = await api.get(`/medicamentos/${props.idMedicamento}`, {
        headers: { Authorization: "Bearer " + token }
      });
      handleClose();
      let dados = getDadosMedicamento.data[0];  
  ...
  async function handleEdit(e) {
    ...
    if (erros.length > 0) {
      ...
      handleOpen();
      try {
        const callBackPost = await api.put(`/medicamentos/${props.idMedicamento}`, data, {
          headers: {
