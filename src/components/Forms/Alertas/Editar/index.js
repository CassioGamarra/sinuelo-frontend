...
export default function FormEditarAlerta(props) {
  ...
  async function buscarAlerta() {
    handleOpen();
    try {
      const getDadosAlerta = await api.get(`/alertas/${props.idAlerta}`, {
        headers: { Authorization: "Bearer " + token }
      });
      handleClose();
      let dados = getDadosAlerta.data[0];  
  ...
  async function handleEdit(e) {
    ...
    if (erros.length > 0) {
      ...
      handleOpen();
      try {
        const callBackPost = await api.put(`/alertas/${props.idAlerta}`, data, {
          headers: {
