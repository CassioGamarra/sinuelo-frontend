...
export default function FormEditarBrinco(props) {
  ...
  async function buscarBrinco() {
    handleOpen();
    try {
      const getDados = await api.get(`/brincos/${props.idBrinco}`, {
        headers: { Authorization: "Bearer " + token }
      });
      handleClose();
      let dados = getDados.data[0];  
      setCodRfid(dados.cod_rfid); 
      setCodVisual(dados.cod_visual); 
      setIdAnimal(dados.id_animal); 
  ...
  async function handleEdit(e) {
    ...
    if (erros.length > 0) {
      ...
      handleOpen();
      try {
        const callBackPost = await api.put(`/brincos/${props.idBrinco}`, data, {
          headers: {
