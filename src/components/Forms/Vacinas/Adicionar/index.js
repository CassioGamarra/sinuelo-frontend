...
export default function FormAdicionarVacina(props) {
  ...
  async function handleRegister(e) {
    ...
    if (erros.length > 0) {
      ...
      handleOpen();
      try {
        const callBackPost = await api.post('/vacinas', data, {
          headers: {
          ...
          <form onSubmit={handleRegister}>
            <Grid container spacing={2} alignItems="flex-end"> 
              ...
              <Grid item xs={12} sm={2}> 
                <FormControlLabel
                  control={<Switch onChange={e => setObrigatorio(e.target.checked)} />}
                  label="Obrigatória" 
                  value={obrigatorio}
                  style={{color:'#FFFFFF'}}
                />
